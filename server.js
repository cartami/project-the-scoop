...
let fs = require('fs');
let writeYamlFile = require('write-yaml-file')
...
function upvote(item, username) {
  if (item.downvotedBy.includes(username)) {
    item.downvotedBy.splice(item.downvotedBy.indexOf(username), 1); //if user downvoted in the past remove user from downvote array
  }
  if (!item.upvotedBy.includes(username)) { //if user is not found in upvote array then push user into array
